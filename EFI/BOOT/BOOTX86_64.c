...
EFI_STATUS efi_main(EFI_HANDLE ImageHandle, EFI_SYSTEM_TABLE *SystemTable)
{
  ...
  EFI_STATUS STATUS = Root->Open(Root, &KernelFile, L"kernel\\kernel.elf", EFI_FILE_MODE_READ, 0);
  if (EFI_ERROR(STATUS)) {
    Print(L"FAILED TO READ THE ELF FILE\n", STATUS);
  }
  ...
  Print(L"parsing files for elf magic numebers\n");
  ...
  for (UINTN i = 0; i < elf->e_phnum; i++) {
    if (phdr[i].p_type == PT_LOAD) {
      ...
      EFI_STATUS status = SystemTable->BootServices->AllocatePages(AllocateAddress, EfiLoaderData, segPages, &segAddr);
