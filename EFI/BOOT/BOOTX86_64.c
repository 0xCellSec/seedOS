...
EFI_STATUS SegmentLoader(struct elf64_ehdr *elf, EFI_PHYSICAL_ADDRESS *buffer, EFI_SYSTEM_TABLE *SystemTable) 
{ 
...
EFI_STATUS efi_main(EFI_HANDLE ImageHandle, EFI_SYSTEM_TABLE *SystemTable)
{
    ...
    if (EFI_ERROR(status))
    {
        Print(L"Something went wrong with the kernel loader: %r\n", status);
        return status;
    }
    ...
    if (EFI_ERROR(status))
    {
        Print(L"Something went wrong with the elf magic check: %r\n", status);
        return status;
    }

    status = SegmentLoader(elf, buffer, SystemTable);
    if (EFI_ERROR(status))
    {
        Print(L"Something went wrong with the SegmentLoader: %r\n", status);
        return status;
    }
